...
export default function LandedCost() {
    const { name, origin, dest, value, weight, qty, transport, hsCode, description, setTradeData } = useTradeContext();
    ...
    useEffect(() => {
        ...
        const fetchCost = async () => {
            ...
            setError(null);
            try {
                // Calculate Total Product Value (Quantity x Unit Price)
                const unitPrice = Number(value?.replace(/[^0-9.-]+/g, "")) || 0;
                const quantity = Number(qty?.replace(/[^0-9.-]+/g, "")) || 1;
        ...
        fetchCost();
    }, [name, origin, dest, value, weight, qty, transport, hsCode, description]);
    ...
    // Derived values
    const units = Number(qty?.replace(/[^0-9.-]+/g, "")) || 1;
    ...
    const costPerUnit = activeTotal / units; 
    ...
    const chartData = [
        { name: "Current Baseline", val: d.total_landed_cost / units, color: "#94a3b8" },
        { name: "Simulated Scenario", val: activeTotal / units, color: "#8b5cf6" },
        { name: "Best Global Option", val: bestScenario.total_landed_cost / units, color: "#10b981" },
    ];
    ...
    return (
        <PageShell title="Landed Cost">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14, marginTop: 24 }}>
                {[
                    { label: "Total Landed Cost", value: fmt(activeTotal), sub: `For ${units.toLocaleString()} units`, border: "#2563eb" },
            ...
            <div className="glass-card card-shadow animate-fade-in-up" style={{ padding: "20px 24px", marginTop: 24, borderRadius: 12 }}>
                <div style={{ marginBottom: 24 }}>
                    <div style={{ fontSize: 16, fontWeight: 700, color: "var(--text-primary)" }}>Cost Optimization Analysis (Per Unit)</div>
                    <div style={{ fontSize: 13, color: "var(--text-muted)", marginTop: 4 }}>Comparison between current and optimized unit-cost scenarios</div>
                </div>
                <ResponsiveContainer width="100%" height={250}>
                    <BarChart data={chartData} margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                        ...
                        <Tooltip formatter={(v: any) => [`$${Number(v).toLocaleString(undefined, { minimumFractionDigits: 2 })}`, "Landed Cost Per Unit"]} cursor={{ fill: 'transparent' }} contentStyle={{ borderRadius: 8, border: "1px solid #e2e8f0", fontSize: 13 }} />
